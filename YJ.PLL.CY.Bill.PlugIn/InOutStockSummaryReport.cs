            ...
            foreach (var orgId in orgIdList)
            {
                ...
                if (i<=1)
                {
                     sql = string.Format(@"/*dialect*/ 
                        SELECT {2} AS FFilterOrgId,T2.FNUMBER AS FSTOCKNO,T1.*
                        INTO {0} FROM {1} T1 INNER JOIN T_BD_STOCK T2 ON T1.FSTOCKID = T2.FSTOCKID", interfaceTableName, reportData.DataSource.TableName, orgId);
                ...
                if (i > 1)
                {
                     sql = string.Format(@"/*dialect*/ 
                        INSERT INTO {0} (
                               FFilterOrgId,FSTOCKNO,FACCTGID,FMATERIALBASEID,FMATERIALID,FMATERIALNAME,FMATERIALGROUP,FMODEL,FLOTNO,FASSIPROPERTYID
	                          ...
	                          ,FPRICEDIGITS,FVALUATION,FISTOTAL,FGROUPBYFIELD,FDETAILREPORTFORMID,FIDENTITYID,FSENDQTY,FSENDPRICE)
                        SELECT {2},T2.FNUMBER AS FSTOCKNO,T1.FACCTGID,T1.FMATERIALBASEID,T1.FMATERIALID,T1.FMATERIALNAME,T1.FMATERIALGROUP,T1.FMODEL,T1.FLOTNO,T1.FASSIPROPERTYID
                              ,FMATERPROPERTY,FMATERTYPE,FBOMNO,FPLANNO,FSEQUENCENO,FPROJECTNO,FOWNERID,FOWNERNAME,FSTOCKORGID
	                          ,FSTOCKORGNAME,T1.FSTOCKID,FSTOCKNAME,T1.FSTOCKPLACEID,FSTOCKPLACENAME,FACCTGRANGEID,FACCTGRANGENAME,FUNITNAME
	                          ,FUNITID,FINITQTY,FINITPRICE,FINITAMOUNT,FRECEIVEQTY,FRECEIVEPRICE,FRECEIVEAMOUNT,FSENDAMOUNT,FENDQTY,FENDPRICE
	                          ,FENDAMOUNT,FSTOCKSTATUSID,FSTOCKSTATUSNAME,FDIMID,FACCTGDIMID,FASSIPROPNAME,FDIGITS,FQTYDIGITS
	                          ,FPRICEDIGITS,FVALUATION,FISTOTAL,FGROUPBYFIELD,FDETAILREPORTFORMID,T1.FIDENTITYID,T1.FSENDQTY,T1.FSENDPRICE
                          FROM {1} T1 INNER JOIN T_BD_STOCK T2 ON T1.FSTOCKID = T2.FSTOCKID", interfaceTableName, reportData.DataSource.TableName, orgId);
                    DBUtils.Execute(this.Context, sql);
                }
